...
// Mapping function: same logic you used in your previous code, but now we do it here
function mapProfileToCsvRows(jsonData) {
  ...
  for (let i = 0; i < 10; i++) {
    if (i < experienceRecords.length) {
      ...
      csvRow[`organization_description_${i + 1}`] = flattenDescription(exp.organization_description);
    } else {
  ...
  csvRow.skills = (jsonData.skills && Array.isArray(jsonData.skills)) 
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Do a single check to see if the snapshot is ready
    ...
    // If it's still running, return JSON indicating that
    if (getResponse.data.status === "running") {
      return res.status(200).json({ status: "running" });
    }

    // Otherwise, let's assume the snapshot is ready and convert it to CSV
    const profileData = getResponse.data;
    const csvContent = mapProfileToCsvRows(profileData);

    // Return the CSV with headers to prompt download
